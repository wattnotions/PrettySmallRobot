...
def sub_cb(topic, msg):
    ...
    wheel = int(data[4])
    print(accel)
    if ((wheel<10) and (wheel>-10) and (accel))  : straight()
    if ((wheel<10) and (wheel>-10) and (accel) and (shift_down))  : reverse()
    if (wheel<-10) and accel  : left()
    if (wheel>10) and accel  : right()
